...
const char* RPI_URL = "http://192.168.0.112:5000/update";
...
void wifictl_send_to_rpi(int steps, float distance) {
    if (!wifictl_get_event(WIFICTL_CONNECT)) {
        log_e("WiFi not connected, skipping HTTP POST");
        return;
    }
    ...
    log_d("HTTP POST response: %d", responseCode);
    http.end();
}
