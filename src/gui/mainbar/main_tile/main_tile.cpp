...
/* -------------------------------------------------------------------------- */
/*  ★ FIX: Async state structs                                                */
/*                                                                             */
/*  Root cause of crash:                                                       */
/*  mainbar_pmu_event_cb(), mainbar_blectl_event_cb(),                        */
/*  mainbar_wifictl_event_cb(), main_tile_sensor_event_cb() and               */
/*  main_tile_time_update_ebent_cb() all call LVGL functions directly from    */
/*  hardware event callbacks that run on different tasks/cores.               */
/*  This races against lv_task_handler() causing heap corruption.             */
/*                                                                             */
/*  Fix: copy the relevant state into a static struct, then use               */
/*  lv_async_call() to defer all LVGL operations to the next                  */
/*  lv_task_handler() tick where they are safe to execute.                    */
/* -------------------------------------------------------------------------- */

// ★ PMU async state
struct PmuAsyncState {
    int32_t pmu_info;
};
static PmuAsyncState pmu_async_state;

// ★ BLE async state
struct BleAsyncState {
    EventBits_t event;
};
static BleAsyncState ble_async_state;

// ★ WiFi async state
struct WifiAsyncState {
    EventBits_t event;
};
static WifiAsyncState wifi_async_state;

// ★ Sensor async state
struct SensorAsyncState {
    float temp;
    float humidity;
};
static SensorAsyncState sensor_async_state = {0.0f, 0.0f};

// ★ Time update async flag


/* -------------------------------------------------------------------------- */
/*  ★ Async LVGL functions — called safely from lv_task_handler              */
/* -------------------------------------------------------------------------- */

static void pmu_update_async( void *data ) {
    if ( !batteryicon || !batterylabel ) return;

    PmuAsyncState *s = (PmuAsyncState *)data;
    int32_t pmu_info = s->pmu_info;
...
static void ble_update_async( void *data ) {
    if ( !bluetoothicon ) return;
    static bool blectl_state = false;

    BleAsyncState *s = (BleAsyncState *)data;
    switch ( s->event ) {
        case BLECTL_ON:
            blectl_state = true;
...
static void wifi_update_async( void *data ) {
    if ( !wifiicon ) return;
    static bool wifictl_state = false;

    WifiAsyncState *s = (WifiAsyncState *)data;
    switch ( s->event ) {
        case WIFICTL_ON:
            wifictl_state = true;
...
static void sensor_update_async( void *data ) {
    if ( !templabel || !infolabel ) return;

    SensorAsyncState *s = (SensorAsyncState *)data;
    char sensor_str[128] = "";
    snprintf( sensor_str, sizeof( sensor_str ),
              "temp/humidity: %0.1f°C/%.0f%%", s->temp, s->humidity );
    lv_label_set_text( templabel, sensor_str );
    lv_obj_align( templabel, infolabel, LV_ALIGN_OUT_TOP_MID, 0, 0 );
}

static void time_update_async( void *data ) {
...
/* -------------------------------------------------------------------------- */
/*  Callbacks — now just copy state and defer to LVGL core via lv_async_call */
/* -------------------------------------------------------------------------- */

static bool mainbar_pmu_event_cb( EventBits_t event, void *arg ) {
    // ★ FIX: Copy PMU state and defer all LVGL calls to lv_task_handler.
    //   Previously called lv_img_set_src() and lv_obj_set_style_*() directly
    //   from PMU callback — this raced against lv_task_handler on Core 1
    //   causing heap corruption (the crash at main_tile.cpp:440).
    switch ( event ) {
        case PMUCTL_STATUS:
            pmu_async_state.pmu_info = *(int32_t *)arg;
            lv_async_call( pmu_update_async, &pmu_async_state );
            break;
    }
    return( true );
}
...
static bool mainbar_blectl_event_cb( EventBits_t event, void *arg ) {
    // ★ FIX: Copy BLE event and defer LVGL icon updates to lv_task_handler.
    //   Previously called lv_obj_set_hidden() and lv_obj_set_style_*()
    //   directly from BLE callback — unsafe cross-task LVGL access.
    ble_async_state.event = event;
    lv_async_call( ble_update_async, &ble_async_state );
    return( true );
}

static bool mainbar_wifictl_event_cb( EventBits_t event, void *arg ) {
    // ★ FIX: Copy WiFi event and defer LVGL icon updates to lv_task_handler.
    //   Previously called lv_obj_set_hidden() and lv_obj_set_style_*()
    //   directly from WiFi callback — unsafe cross-task LVGL access.
    wifi_async_state.event = event;
    lv_async_call( wifi_update_async, &wifi_async_state );
    return( true );
}

static bool main_tile_style_event_cb( EventBits_t event, void *arg ) {
    // Style updates only happen from UI context — safe to call directly
    switch ( event ) {
        case STYLE_CHANGE:
            style = ws_get_mainbar_style();
            lv_style_copy( &timestyle, style );
...
static bool main_tile_sensor_event_cb( EventBits_t event, void *arg ) {
    // ★ FIX: Copy sensor values and defer lv_label_set_text() to
    //   lv_task_handler. Previously called directly from sensor callback.
    switch ( event ) {
        case SENSOR_TEMPERATURE:
            sensor_async_state.temp = *(float *)arg;
            break;
        case SENSOR_RELHUMIDITY:
            sensor_async_state.humidity = *(float *)arg;
            break;
    }
    lv_async_call( sensor_update_async, &sensor_async_state );
    return( true );
}
...
static bool main_tile_time_update_ebent_cb( EventBits_t event, void *arg ) {
    // ★ FIX: Defer time update to lv_task_handler instead of calling
    //   main_tile_update_time() directly from timesync callback.
    switch ( event ) {
        case TIME_SYNC_UPDATE:
            lv_async_call( time_update_async, NULL );
            break;
    }
    return( true );
}
...
static bool main_tile_powermgm_event_cb( EventBits_t event, void *arg ) {
    ...
    switch ( event ) {
        case POWERMGM_WAKEUP:
            // ★ FIX: Defer time update to lv_task_handler instead of calling
            //   main_tile_update_time() directly from powermgm callback.
            lv_async_call( time_update_async, NULL );
            break;
    }
    return( true );
...
void main_tile_align_widgets( void ) {
    ...
    for ( int widget = 0; widget < MAX_WIDGET_NUM; widget++ ) {
        if ( widget_entry[ widget ].active ) {
            lv_obj_align( widget_entry[ widget ].icon_cont, main_cont, LV_ALIGN_IN_BOTTOM_MID,
                          xpos + ( WIDGET_X_SIZE * active_widgets ) + ( active_widgets * WIDGET_X_CLEARENCE ) + 32,
                          -( ( lv_disp_get_ver_res( NULL ) / 4 ) - 32 ) );
            active_widgets++;
        }
    }
}
...
/* -------------------------------------------------------------------------- */
/*  Time update — always called from LVGL task context                        */
/* -------------------------------------------------------------------------- */
